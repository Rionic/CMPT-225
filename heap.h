...
void Heap::trickleDown(int i){
   // Complete this.  int min = 2*i+1;
   if (hSize==1 || i>=hSize-1)
      return;
   else{
      int left = 2*i+1;
      int right = 2*i+2;
      ...
      else if(A[left].priority<=A[right].priority)
         min = left;
      else if(A[left].priority>=A[right].priority)
         min = right;
   ...
   // if(i>=hSize-1 || hSize==1){ 
   //    return;  
   // }
   // else{
   //    int min = 2*i+1;
   //    if(A[min].priority>=A[2*i+2].priority){
   //       min = 2*i+2;
   //    }
   //    else if(A[min].priority<A[2*i+2].priority){
   //       min = 2*i+1;
   //    }
   //    if(A[i].priority>A[min].priority){
   //       swap(i,min);
